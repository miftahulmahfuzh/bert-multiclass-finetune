...
def load_config(config_path="finetune_config.json"):
    ...
    if "dataset" not in config:
        config["dataset"] = {}
    if "use_test_for_validation" not in config["dataset"]:
        config["dataset"]["use_test_for_validation"] = True
    if "text_column" not in config["dataset"]:
        config["dataset"]["text_column"] = "text"
    if "label_column" not in config["dataset"]:
        config["dataset"]["label_column"] = "label"

    print(json.dumps(config, indent=3))

    # Load dataset and get label information
    dataset = load_dataset(config["dataset"]["name"])

    # Get unique labels from the training set
    if 'train' in dataset:
        unique_labels = set(dataset['train'][config["dataset"]["label_column"]])
        id2label = {str(label)[0]: str(label)[0] for i, label in enumerate(unique_labels) if label is not None}
        label2id = {str(v): int(k) for k, v in id2label.items()}
    else:
        raise ValueError(f"Dataset {config['dataset']['name']} does not have a 'train' split")
...
class CachedHFDataset(Dataset):
    def __init__(self, hf_dataset, tokenizer, max_length, cache_path=None, split="", CONFIG={}):
        self.cache_path = cache_path
        self.tokenized_data = None
        self.CONFIG = CONFIG
        self.text_column = CONFIG["dataset"]["text_column"]
        self.label_column = CONFIG["dataset"]["label_column"]

        # Get the texts and labels
        self.texts = hf_dataset[self.text_column]
        self.labels = hf_dataset[self.label_column]
        ...
        if cache_path and os.path.exists(cache_path) and self.CONFIG["paths"]["use_cache"]:
            ...
            self.tokenized_data = self._tokenize_and_cache(tokenizer, max_length)

    def _tokenize_and_cache(self, tokenizer, max_length):
        tokenized_data = []
        for text, label in tqdm(zip(self.texts, self.labels), total=len(self.texts), desc="Tokenizing", disable=dist.get_rank() != 0):
            is_empty = True
            ...
            tokenized_data.append(item_dict)
...
def load_and_prepare_data(rank, world_size, CONFIG):
    ...
    if rank == 0:
        print(f"Loaded dataset splits: {dataset.keys()}")
    ...
    # Use test set for validation if specified
    use_test_for_validation = CONFIG["dataset"].get("use_test_for_validation", True)
    if use_test_for_validation:
        ...
        if 'validation' not in dataset:
            raise ValueError("Dataset does not have a validation split and use_test_for_validation is False")
    ...
    val_sampler = test_sampler if use_test_for_validation else DistributedSampler(val_dataset, num_replicas=world_size, rank=rank)
    ...
    val_dataloader = test_dataloader if use_test_for_validation else DataLoader(
        val_dataset,
        batch_size=CONFIG["training"]["batch_size"],
        sampler=val_sampler,
        num_workers=2,
    ...
    return train_dataloader, val_dataloader, test_dataloader, tokenizer, dataset
...
def evaluate(
    ...
    # Get the correct split and column names
    if not is_test and dataset is not None and "use_test_for_validation" in config:
        if config["use_test_for_validation"]:
            split = 'test'
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc=desc, disable=dist.get_rank() != 0)):
            ...
            # Store the original texts from the dataset
            if dataset is not None:
                start_idx = batch_idx * dataloader.batch_size
                end_idx = start_idx + len(predictions)
                all_texts.extend(dataset[split][text_column][start_idx:end_idx])

            del outputs, predictions
            torch.cuda.empty_cache()

    # Rest of the evaluate function remains the same...
...
def train(rank, world_size, CONFIG):
    ...
    # Load data
    train_dataloader, val_dataloader, test_dataloader, tokenizer, dataset = load_and_prepare_data(rank, world_size, CONFIG)
