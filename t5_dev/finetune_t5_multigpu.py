...
def evaluate(model, dataloader, device, target_names=None, is_test=False):
    model.eval()
    total_loss = 0
    all_preds = []
    all_labels = []
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=desc, disable=dist.get_rank() != 0):
            ...
            generated_ids = model.generate(
                input_ids=input_ids,
                attention_mask=attention_mask,
                max_length=10  # Adjust based on label length
            )
    ...
    if dist.is_initialized():
        if dist.get_rank() == 0:
            all_preds_gathered = [None for _ in range(dist.get_world_size())]
            all_labels_gathered = [None for _ in range(dist.get_world_size())]
        else:
            all_preds_gathered = None
            all_labels_gathered = None

        gather_list_preds = [torch.tensor([], dtype=torch.int32).to(device) for _ in range(dist.get_world_size())]
        gather_list_labels = [torch.tensor([], dtype=torch.int32).to(device) for _ in range(dist.get_world_size())]

        # Convert string predictions and labels to tensors for gathering
        if dist.get_rank() == 0:
            all_preds_str = all_preds
            all_labels_str = all_labels
        else:
            all_preds_str = []
            all_labels_str = []

        # Only the main process will handle metrics
        if dist.get_rank() == 0:
            ...
            report_dict = classification_report(
                all_labels_final,
                all_preds_final,
                labels=target_names,  # Use label texts
            ...
            if 'accuracy' not in report_df.index:
                ...
                report_df.at['accuracy', 'support'] = len(all_labels_final)
        else:
            accuracy = 0
            report_df = None
...
def train(rank, world_size, CONFIG):
    ...
    for epoch in range(CONFIG["training"]["epochs"]):
        ...
        if rank == 0:
            if accuracy > best_accuracy:
                ...
                # Save evaluation for every best checkpoint in case the training crashed mid-run
    ...
    # After training, evaluate on test data using the best model
    if rank == 0:
        print("\nEvaluate best model on test data")

    # Load the best model
    model.module.load_state_dict(torch.load(os.path.join(CONFIG["paths"]["output_dir"], CONFIG["model"]["name"] + "/pytorch_model.bin"), map_location=device))
