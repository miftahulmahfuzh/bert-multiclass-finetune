...
def evaluate_predictions(trainer, config, tokenizer):
    ...
    with torch.no_grad():
        for item in tqdm(test_dataset):
            ...
            text = item['text'].split("Input: ")[1].split("Response:")[0].strip()
            texts.append(text)
...
def main():
    ...
    if config["training"]["mode"] == "test":
        ...
        # print("Loading trained adapter model...")
        # model = AutoModelForCausalLM.from_pretrained(config["paths"]["trained_adapter"])
